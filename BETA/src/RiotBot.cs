...
namespace RitoBot
{
    internal class RiotBot
    {
        ...
        public PVPNetConnect.RiotObjects.Platform.Catalog.Champion.ChampionDTO[] availableChampsArray;
        ...
        public RiotBot(string username, string password, string reg, string path, int threadid, QueueTypes QueueType)
        {
            ...
            queueType = QueueType;
            region = reg;
            connection.OnConnect += new PVPNetConnection.OnConnectHandler(this.connection_OnConnect);
            connection.OnDisconnect += new PVPNetConnection.OnDisconnectHandler(this.connection_OnDisconnect);
            connection.OnError += new PVPNetConnection.OnErrorHandler(this.connection_OnError);
            connection.OnLogin += new PVPNetConnection.OnLoginHandler(this.connection_OnLogin);
            connection.OnLoginQueueUpdate += new PVPNetConnection.OnLoginQueueUpdateHandler(this.connection_OnLoginQueueUpdate);
            connection.OnMessageReceived += new PVPNetConnection.OnMessageReceivedHandler(this.connection_OnMessageReceived);
            switch (region)
            {
                case "EUW":
                ...
                case "TR":
                    ...
                    break;
                case "LAS":
                    connection.Connect(username, password, Region.LA2, Program.cversion);
                    break;
                case "LAN":
                    connection.Connect(username, password, Region.LA1, Program.cversion);
                    break;
            }
        }
        ...
        public async void connection_OnMessageReceived(object sender, object message)
        {
            if (message is GameDTO)
            {
                ...
                switch (game.GameState)
                {
                    ...
                    case "JOINING_CHAMP_SELECT":
                        if (this.firstTimeInQueuePop && game.StatusOfParticipants.Equals("1"))
                        {
            ...
            else if (message is PlayerCredentialsDto)
            {
                ...
                PVPNetConnect.RiotObjects.Platform.Game.PlayerCredentialsDto credentials = message as PlayerCredentialsDto;
                System.Diagnostics.ProcessStartInfo startInfo = new System.Diagnostics.ProcessStartInfo();
                startInfo.CreateNoWindow = false;
                startInfo.WorkingDirectory = str;
                startInfo.FileName = "League of Legends.exe";
                startInfo.Arguments = "\"8394\" \"PVPNetConnect.exe\" \"\" \"" + credentials.ServerIp + " " +
            ...
            else if (!(message is GameNotification) && !(message is SearchingForMatchNotification))
            {
                ...
                {
                    PVPNetConnect.RiotObjects.Platform.Matchmaking.MatchMakerParams matchParams = new PVPNetConnect.RiotObjects.Platform.Matchmaking.MatchMakerParams();
                    ...
                    PVPNetConnect.RiotObjects.Platform.Matchmaking.SearchingForMatchNotification m = await connection.AttachToQueue(matchParams);
        ...
        private void connection_OnLogin(object sender, string username, string ipAddress)
        {
            ...
            { 
                ...
                if (loginPacket.AllSummonerData == null)
                {
                    ...
                    PVPNetConnect.RiotObjects.Platform.Summoner.AllSummonerData sumData = await connection.CreateDefaultSummoner(summonerName);
                ...
                PVPNetConnect.RiotObjects.Team.Dto.PlayerDTO player = await connection.CreatePlayer();
                if (this.loginPacket.ReconnectInfo != null && this.loginPacket.ReconnectInfo.Game != null)
                {
                    this.connection_OnMessageReceived(sender, (object)this.loginPacket.ReconnectInfo.PlayerCredentials);
                }
        ...
        private void connection_OnError(object sender, PVPNetConnect.Error error)
        {
